...
use Tecnodesign_Form_Field as Field;
...
class Model implements ArrayAccess, Iterator, Countable
{
    ...
    public static function staticInitialize()
    {
        ...
        if(static::$schema && static::$auditLog) {
            static::$schema->audit = static::$auditLog;
        }
    ...
    public function __sleep()
    {
        ...
        $not = [ '_forms', '_collection', '_delete', '_query', '_original', '_p' ];
     ...
     *
     * loads the schema if it is a Studio\Model object, or builds the schema and stores it in cache, if it is from another class
     * Syntax: Model::schema($cn=null, $base=array())
     *
    ...
    public static function schema($cn=null, $base=array())
    {
        ...
        if(isset($cn::$schema)) {
            $schema = $cn::$schema;
        } else if(!($schema=Cache::get('schema/'.$cn, S::$timeout))) {
            $schema = $base;
            if(!isset($schema['tableName']) || $schema['tableName']=='') {
                $schema['tableName'] = S::uncamelize(lcfirst($cn));
            }
            $dbold = S::$database;
            ...
            if(!isset($schema['database']) || $schema['database']=='') {
                ...
                // set default database for S::query
                $db = $schema['database'];
            ...
            if($S = static::sourceSchema($schema['tableName'])) {
                ...
                $schema = new ${static::$schemaClass}($schema);
            }
            Cache::set('schema/'.$cn, $S, S::$timeout);
        }
    ...
    public static function sourceSchema($tn=null, $base=[])
    {
        ...
        if($schema=$H->getTableSchema($tn)) {
            return new ${static::$schemaClass}($schema + $base);
        }
    }
    ...
    public function getPk($array=null)
    {
        ...
        foreach($pk as $fn) {
            ...
            $r[$b.$fn]=(!isset($this->$fn) && method_exists($this, $m='get'.S::camelize($fn, true)))?($this->$m()):($this->$fn);
        }
        if($array) return $r;
        ...
        if(!static::$schema->overlay) {
            ...
            if(static::$schema->properties) {
                foreach(static::$schema->properties as $fn=>$fd) {
                    ...
                    if(!isset(static::$schema->overlay[$fn]['bind'])) static::$schema->overlay[$fn]['bind'] = $fn;
                    if (isset($fk[$fn])) {
        ...
        if($scope) {
            ...
            foreach($scope as $label=>$fn) {
                ...
                if(isset($fd['label']) && substr($fd['label'], 0, 1)=='*')  {
                    ...
                    $label = $fd['label'] = S::t(substr($fd['label'], 1), $translate);
                }

                if(isset($fd['fieldset']) && substr($fd['fieldset'], 0, 1)=='*')  {
                    if(!isset($translate)) $translate = 'model-'.static::$schema->tableName;
                    $fd['fieldset'] = S::t(substr($fd['fieldset'], 1), $translate);
                }
            ...
            foreach($scope as $k=>$fn) {
                ...
                if(isset(static::$schema->properties[$fn])) {
                    if(is_array(static::$schema->properties[$fn])) {
                        static::$schema->properties[$fn] = new ModelProperty(static::$schema->properties[$fn]);
                    }
                    $fd += static::$schema->properties[$fn]->value();
                }
                ...
                if(isset($fd['label']) && substr($fd['label'], 0, 1)=='*')  {
                    if(!isset($translate)) $translate = 'model-'.static::$schema->tableName;
                    $fd['label'] = S::t(substr($fd['label'], 1), $translate);
                }
    ...
    public static function column($s, $applyForm=false, $relation=false)
    {
        ...
        if($d && isset($d['label']) && substr($d['label'], 0, 1)=='*')  {
            if(!isset($translate)) $translate = 'model-'.static::$schema->tableName;
            $d['label'] = S::t(substr($d['label'], 1), $translate);
        }
    ...
    public function getForm($scope=null, $pk=false, $parentForm=null)
    {
        ...
        if (!isset($this->_forms[$hash]) || !($F=Form::getInstance($this->_forms[$hash]))) {
            $cn = get_called_class();
    ...
    public function refresh($scope=null)
    {
        ...
        if($f) {
            if($M = $this::find($this->getPk(true),1,$f)) {
                foreach($f as $p=>$fn) {
                    if(isset($M->$p)) {
                        if(isset($this::$schema->properties[$p]) && !isset($this->_original[$p])) {
        ...
        if($scope && is_array($scope)) {
            foreach($scope as $fn=>$fv) {
                ...
                if(!is_null($v) || $valueFormat!==false) {
                    ...
                    if(isset($schema->properties[$fn]) && $schema->properties[$fn]->serialize) {
                        if(is_array($v) && $serialize) {
                            $v = S::serialize($v, $schema->properties[$fn]->serialize);
                        } else if($serialize===false && is_string($v)) {
                            $nv = S::unserialize($v, $schema->properties[$fn]->serialize);
                            if($nv!==false && $nv!==null) $v = $nv;
                        }
                    }
                    if(!S::isempty($v) || $valueFormat!==false) {
                        $result[$k] = $v;
                    }
                    unset($k, $v);
                }
    ...
    public static function relate($r, &$f=null, $addRef=false, $rp='')
    {
        ...
        if($f) {
            ...
            if(!isset($rrn)) {
                ...
                $rrn = 'r'.$i;
                $rcn::$schema->relations[$rrn] = [
                    'className'=>$cn,
                    'foreign'=>$ro['local'],
                    'local'=>$ro['foreign'],
                    'type'=>($ro['type']=='one')?('many'):('one')
                ];
            } else {
                unset($rr);
            }
    ...
    public function getRelationQuery($relation, $part=null, $scope=null)
    {
        ...
        while($rn) {
            ...
            if(!isset($sc->relations[$reln])) {
                throw new AppException(array(S::t('Relation "%s" is not available at %s.','exception'), $reln, $rcn));
            }
    ...
    public function getRelation($relation, $fn=null, $scope=null, $asCollection=true)
    {
        ...
        if (!isset(static::$schema->relations[$relation])) {
            throw new AppException(array(S::t('Relation "%s" is not available at %s.','exception'), $relation, get_called_class()));
        }
        $rel = static::$schema->relations[$relation];
        $rcn = (isset($rel['className']))?($rel['className']):($relation);
        if (!class_exists($rcn)) {
            throw new AppException(array(S::t('Class "%s" does not exist.','exception'), $rcn));
        }
        ...
        if($p && !isset($rcn::$schema->properties[$fn])) {
            $relation .= '.'.$fn;
            $fn = $fn0;
        }
        ...
        if(!isset($this->$relation) || !isset($this->_relation[$relation]) || $this->_relation[$relation]!=$rk) {
            ...
            $Q = null;
            if($fn && !isset($rcn::$schema->properties[$fn])) {
            ...
            if(!$Q) {
                ...
                $this->$relation = new Collection(null, $Q->schema('className'), $Q, null);
            } else {
        ...
        if($fn) {
            if(isset($this->$relation) && ($R=$this->$relation)) {
                if(is_object($R) && ($R instanceof Collection)) $R = $R->getItems();
        ...
        } else if(!$asCollection && is_object($this->$relation) && $this->$relation instanceof Collection) {
    ...
    public function setRelation($relation, $value, $raw=false)
    {
        ...
        if($this->$relation instanceof Collection) { // if it's a collection, expand
            $this->$relation = $this->$relation->getItems();
            if(!$this->$relation) $this->$relation = array();
        } else if($rel['type']=='many') {
            if($this->$relation instanceof Model) $this->$relation = array($this->$relation);
         ...
         */
        if($value instanceof Collection) { // if it's a collection, expand
    ...
    public function save($beginTransaction=null, $relations=null, $conn=false)
    {
        ...
        if(S::$perfmon) $perfmon = microtime(true);
        try {
            ...
            if(!$this->runEvent('before-'.$m, $conn)) {
                throw new AppException(array(S::t('Could not '.$m.' record at %s.', 'exception'), $cn::label()));
            }
            if ($this->_delete) {
                $m = 'delete';
            } else if ($this->_new) {
            ...
            if($relations && $cn::$schema->relations) {
                ...
                foreach ($cn::$schema->relations as $rcn=>$rd) {
                    ...
                    if(!S::classFile($rc) || !isset($rc::$schema) || $rc::$schema->type==='view') {
                        continue;
                    }
                    if(isset($this->$rcn)) {
                        $rel = $this->$rcn;
                        if(is_object($rel) && ($rel instanceof Collection)) {
                            unset($rel);
                            continue;
                        }
            ...
            if(!(substr($msg, 0, 1)=='<' && strpos(substr($msg, 0, 100), 's-i-msg'))) {
                if($msg) $msg = "\n".S::t('Issues are', 'exception').":\n".$msg;
                $msg = array(S::t('Could not save %s.', 'exception').$msg, $cn::label());
            }
        ...
        if(S::$perfmon>0) S::log('[INFO] '.get_called_class().'::save(): '.S::number(microtime(true)-$perfmon).'s '.S::bytes(memory_get_peak_usage())." mem");
    ...
    public function auditLog($action, $pk, $data=null)
    {
        ...
        if(!is_array($a) || !isset($a['type']) || $a['type']==='log') {
            // save to log
            if((is_string($a) && (file_exists($f=S_VAR.'/'.$a) || touch($f=S_APP_ROOT.'/'.$a)))
                || (is_array($a) && isset($a['file']) && (file_exists($f=S_VAR.'/'.$a['file']) || touch($f=S_APP_ROOT.'/'.$a['file'])))) {
                error_log(S::serialize($log, 'json')."\n", 3, $f);
            } else {
                S::log($log);
            }
            ...
            S::log('[INFO] Unknown audit log option: '.S::serialize($a, 'json'), $log);
        }
    }
    ...
    public static function fieldLabel($fn, $translate=true)
    {
        ...
        if(!isset($cn::$schema['form'][$fn]['label'])) {
            $label = trim(ucwords(strtr(S::uncamelize($fn), '-_', '  ')));
            if ($translate) {
    ...
    public static function find($s=null, $limit=0, $scope=null, $collection=true, $orderBy=null, $groupBy=null)
    {
        ...
        if(!$Q) {
            ...
            return new Collection(null, get_called_class(), $Q, null);
        }
    }
    ...
    public function renderScope($scope=null, $xmlEscape=true, $box=null, $tpl=null, $sep=null, $excludeEmpty=null, $showOriginal=null)
    {
        ...
        $id = $scope;
        $s = '';
        if(!is_array($scope) && method_exists($this, $m='renderScope'.S::camelize($scope, true))) {
            $s = $this->$m();
            $scope = array();
                ...
                if(preg_match('/^([a-z0-9\-\_]+)::([a-z0-9\-\_\,]+)(:[a-z0-9\-\_\,\!]+)?$/i', $fn, $m)) {
                    ...
                    if($showOriginal && array_key_exists($fn, $this->_original)) {
                        ...
                        if($v0!=$v1) {
                            ...
                            if($nv!=$v) {
                                $v = '<span class="s-m-original">'.$nv.'</span>'
                                   . '<span class="s-m-value">'.$v.'</span>'
                                   ;
                            }
                            unset($nv);
                            $this->$fn = $v1;
    ...
    public function renderUi($o=array())
    {
        ...
        $sf = null;
        if(!is_array($link) && isset(S::$variables['Interface']) && is_object(S::$variables['Interface'])) {
            $I = S::$variables['Interface'];
            $sf = App::request('get', $I::REQ_ORDER);
            ...
            $qsb=($qsb)?('?'.$qsb.'&'):('?');
            if(!(($sf=App::request('get', 'o')) && is_numeric($sf) && $sf<count($labels))) {
                $sf = (isset($schema->orderBy))?(array_keys($schema->orderBy)):(array(''));
                $sf=$sf[0];
            }
            if(!(($sd=App::request('get', 'd')) && ($sd=='asc' || $sd=='desc'))) {
        ...
        if($i==$start) {
            ...
            foreach($labels as $label=>$fn) {
                ...
                if(is_numeric($label)) $label = S::t(ucwords(str_replace('_', ' ', $fid)), 'model-'.$model);
                else if(substr($label,0,1)=='*') $label = S::t(substr($label,1), 'model-'.$model);
                ...
                if($sort && isset($o['show-sort-icons']) && $o['show-sort-icons']) {
                    ...
                    $attrs = '';
                    $asort =  S::scriptName().$ext.S::xml($qsb.$soa);
                    if($sf==$sc) {
        ...
        foreach($labels as $label=>$fn) {
            ...
            $lattrs = null;
            if($fullurl && isset($o['link-class'])) $lattrs = ' class="'.S::xml($o['link-class']).'"';

            $s .= '<td class="f-'.$fn.' '.(($uid!==false && $checkbox)?(' s-check'):('')).'">'
                . (($uid!==false && $checkbox)?('<input type="'.$checkbox.'" id="uid-'.S::xml($this->getPk()).'" name="uid'.(($checkbox==='checkbox')?('[]'):('')).'" value="'.$uid.'" />'):(''))
                . (($fullurl)?('<a href="'.S::xml($fullurl).'"'.$lattrs.'>'.$value.'</a>'):($value))
                .'</td>';
    ...
    public function renderField($fn, $fd=null, $xmlEscape=false)
    {
        ...
        if(method_exists($this, $pm)) {
            ...
            $cI=Api::current();
            $icn = ($cI)?(get_class($cI)):(Api::$className);
            ...
            $I->setAction($a);
            $I->setUrl(S::scriptName(true).'/'.$fd['interface']);
            $v = $I->preview();
            $xmlEscape = false;
        } else if(isset($fd['type']) && $fd['type']=='file' && isset($fd['accept']['inline-preview']) && $fd['accept']['inline-preview'] && ($f=Image::base64Data($this[$fn]))) {
        ...
        if(!$getRef) {
            ...
            if(isset($fd['serialize']) && is_string($v) && substr($v, 0, 1)==='[' && substr($v, -1)===']') {
                $v = S::unserialize($v, $fd['serialize']);
        ...
        if(isset($fd['choices'])) {
            ...
            if(is_string($choices)) {
                ...
                if(class_exists($choices)) {
                    ...
                    if($val instanceof Model) $val = (string) $val;
        ...
        if(substr($name, 0, 6)=='findBy' && strlen($name)>6) {
            ...
            foreach($name as $cfn) {
                if ($cfn=='Or') {
                    ...
                    $fn = $pipe.S::uncamelize(lcfirst($cfn));
    ...
    public function __get($name)
    {
        ...
        if (method_exists($this, $m)) {
            ...
            if(method_exists($this, $m='get'.S::camelize($firstName, true))) {
                ...
                $this->$firstName = S::unserialize($this->$firstName, static::$schema->properties[$firstName]->serialize);
                $a = $this->$firstName;
            } else {
                $a = $this->$firstName;
            }
    ...
    public function safeSet($name, $value, $skipValidation=false)
    {
        ...
        } else if($firstName && $ref && method_exists($this, $m='set'.S::camelize($firstName, true))) {
