...
class Excel extends SchemaObject
{
    ...
    public function language(string $lang): bool
    {
        ...
        return Settings::setLocale($lang);
    }
    ...
    private function init(): void
    {
        ...
        if (isset($this->template) && $this->template) {
             ...
             * ods = OOCalc 
             */            
            $sheettype = IOFactory::identify($this->template);            
            $reader = IOFactory::createReader($sheettype);		
    ...
    public function page(array $p=array()): void
    {
        $ms = ['setup'=>'getPageSetup', 'margin'=>'getPageMargins'];
        foreach($p as $k=>$v){
     ...
     * Set Row Height
     */
    public function setRowHeight(string $row, int $tam): void
    {
     ...
     * Set Col Width
     */
    public function setColWidth(string $col, int $tam): void 
    {
        if (is_int($col)) {
            $col = S::numberToLetter($col);
        }
    ...
    public function addData($data, $purge=false): void
    {
        $this->data += $data;
        if($purge) {
            $this->compositeExcel();
    ...
    public function getColLetter(int $idx): string
    {
        return S::numberToLetter($idx, true);
    }
    
    private function getColIndex(string $letter): int
    {
        return S::letterToNumber($letter);
    }

    public static function cell(array|string $c, string|null $to=null): string
    {
        if(is_array($c)) {
            if($c[1]<0)$c[1]=0;
            $c = strtoupper(S::numberToLetter((int)$c[0])) . ($c[1]+1);
    ...
    public function merge(string $from, string|null $to=null): void
    {
        $this->sheet->mergeCells(self::cell($from, $to));
    }

    public static function image(string $v, array|null &$s=array(), bool $headerFooter=false): HeaderFooterDrawing|Drawing
    {
        //if($s && ((isset($s['height']) && !$s['height'])||(isset($s['width']) && !$s['width']))) return false;
        $img = ($headerFooter)?(new HeaderFooterDrawing()):(new Drawing());
    ...
    private function compositeExcel() 
    {
        ...
        foreach($this->data as $r => $c) {
            ...
            $colidx = S::letterToNumber($this->col);
            foreach ($c as $ck => $cv) {
                if (is_string($ck) && preg_match('/^[A-Z]+$/', $ck)) {
                    $this->col = $ck;
                    $colidx = S::letterToNumber($this->col);
                } else {
                    $this->col = S::numberToLetter($colidx, true);
                    $colidx++;
                }
    ...
    public function addStylesheet($style=null, $add=true)
    {
        ...
        if(!is_null($style)) {
            $style = self::parseCss($style);
            if($this->style) $this->style = ($add)?(S::mergeRecursive($style, $this->style)):(S::mergeRecursive($this->style, $style));
    ...
    public function render($format = 'xlsx', $filename= '', $download=true, $keepFile=false) 
    {
        ...
        if(method_exists($this, $m='render'.S::camelize($format, true))) {
    ...
    private function download($download, $file, $keepFile=false)
    {
        if($download) {
            ...
            S::download($file, null, $fname, 0, true, false, false);
    ...
    public function addContent($c)
    {
        ...
        if(isset($c['content']) && isset($c['method']) && is_array($c['content'])) {
            ...
            if(isset($c['arguments'])) {
                ...
                $v = S::call($call, $c['arguments']);
                unset($c['arguments']);
            } else {
                $v = S::call($call);
            }
            unset($o, $call);
        } else if(isset($c['content']) && is_object($c['content']) && $c['content'] instanceof Collection) {
            if(isset($c['arguments'])) {
                foreach($c['arguments'] as $a=>$b) {
                    $c['arguments'][$a] = $this->val($b);
                    unset($a, $b);
                ...
                S::objectCall($this, 'addCollection', $c['arguments']);
                unset($c['arguments']);
            } else {
        ...
        if(isset($c['type'])) {
            if($c['type']=='formula') $t = static::TYPE_FORMULA;
                ...
                $t = static::TYPE_IMAGE;
                $v = S::getApp()->app['document-root'].'/'.$v;
            } else if($c['type']=='loop') {
