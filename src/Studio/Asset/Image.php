...
class Image {
    ...
    public function addSettings($config=array())
    {
        ...
        if(isset($config['font'])) {
            ...
            else if(file_exists(S_ROOT.'/src/fonts/'.$config['font'])) $this->font=$config['font'];
        }
        ...
        if(isset($config['text'])) {
            ...
            if($this->font) {
                ...
                else if(file_exists(S_ROOT.'/src/fonts/'.$this->font)) $fontfile = S_ROOT.'/src/fonts/'.$this->font;
            }
            $tx=$x;
    ...
    public static function fileSrc($src)
    {
        if(!file_exists($src)) {
            ...
            foreach(self::$checkSrc as $k) {
                if(($d=App::config($k)) && file_exists($d.'/'.$o)) {
                    $src = $d.'/'.$o;
                    break;
                }
            }
    ...
    public static function base64Data($img)
    {
        ...
        if(!$img) {
            ...
            $fpart = explode('|', $img);
            if($fpart && file_exists($f=S::uploadDir().'/'.preg_replace('/[^a-z0-9A-Z\.\-\_]/', '', $fpart[0]))) {
                $format = S::fileFormat($f);
