...
use Studio\Cache;
use Studio\Model;
use Studio\Form\Field as Field;
...
class Form implements ArrayAccess //, Iterator, Countable
{
    public static 
        ...
        $defaultLimits  = [
            ...
            'key'           => 'session+ip',// how to key the rate limiting, options are 'session', 'ip', 'user', 'url' or a combination of these 
                                            // (use + to rate limit both individually, | for combined)
            'fieldname'     => '__req',     // field name to include for rate limiting
            'fieldset'      => null,        // should rate-limiting be rendered within fieldsets? 
                                            // enter false to remove or the fieldset name to isolate in a proper fieldset
            'attributes'    => null,        // attributes the fieldset field should use
            'timeout'       => 14400,       // how long to keep keys for rate limiting, also expires form after this number of seconds
            ...
            'accept-failed' => true,        // whether the form data should be accepted if it fails 
            'skip-actions'  => [],          // actions to skip in limit checking
            'recaptcha'     => null,        // recaptcha secret key
        ];
    ...
    public function __construct($formConfig=[])
    {
        ...
        if (isset($formConfig['fields'])) {
            ...
            foreach ($formConfig['fields'] as $fieldName => $fieldDefinition) {
                ...
                $this->fields[$fieldName] = new Field($fieldDefinition, $this);
        ...
        if(static::$assets) {
            if(is_array(static::$assets)) {
                App::$assets = array_unique(array_merge(App::$assets, static::$assets));
            } else {
    ...
    public function checkLimits($post=null)
    {
        ...
        if(isset($this->limits['recaptcha']) && $this->limits['recaptcha']) {
            ...
            else if(isset(S::$variables['recaptcha-secret-key'])) $rckey = S::$variables['recaptcha-secret-key'];

            if($rckey) {
                $rcfail = null;
                if(!isset($post['g-recaptcha-response'])) $rcfail = true;
                else {
                    $Q = QueryApi::runStatic('https://www.google.com/recaptcha/api/siteverify', '', http_build_query(['secret'=>$rckey, 'response'=>$post['g-recaptcha-response']]));

                    if(!$Q || !isset($Q['success']) || !$Q['success']) {
                        $rcfail = true;
                        S::log('[INFO] Failed reCAPTCHA validation: '.S::serialize($Q, 'json'));
                    }
                }
                if($rcfail) {
                    $this->limits['error'] = S::t('You need to confirm you\'re not a robot to continue.', 'exception');
                    return false;
        ...
        if($post && isset($post[$this->limits['fieldname']]) && ($pk=$post[$this->limits['fieldname']])) {
            ...
            if(($salt=Cache::get('f-limit-current/'.$k, $timeout)) && $pk==$salt) {
                Cache::delete('f-limit-current/'.$k);
                $salt = S::salt();
                Cache::set('f-limit-current/'.$k, $salt, $timeout);
                $add[$salt] = [$now,0];
            ...
            foreach($this->limits['keys'] as $k=>$ks) {
                ...
                if(isset($ks[$pk]) && $ks[$pk][1]==0 && $ks[$pk][0] + $time >= $now) {
                    ...
                    Cache::set('f-limit/'.$k, $ks, $timeout);
                } else {
                    $valid = false;
                }
            }
        ...
        $this->limits['error'] = S::t('This request is expired or invalid. Please try sending this request again.', 'exception');
        return false;
    }
    ...
    public function getLimits()
    {
        ...
        if(!isset($this->limits['keys'])) {
            ...
            foreach($keys as $k) {
                ...
                if(in_array('ip', $ks)) $k .= '_'.(string)App::request('ip');
                if(in_array('url', $ks)) $k .= '__'.S::scriptName(true);

                $rks = Cache::get('f-limit/'.$k, $timeout);
                if(!$rks) $rks = [];
                $this->limits['keys'][$k]=$rks;
            }

            $k = md5(implode(':', array_keys($this->limits['keys'])));
            if(!($salt=Cache::get('f-limit-current/'.$k, $timeout))) {
                $salt = S::salt();
                Cache::set('f-limit-current/'.$k, $salt, $timeout);
            }
            ...
            foreach($this->limits['keys'] as $k=>$ks) {
                ...
                Cache::set('f-limit/'.$k, $rks, $timeout);
            }
            $fn = $this->limits['fieldname'];
            $csrf = ['id'=>$fn, 'value'=>$salt, 'type'=>'hidden'];
            if(isset($this->limits['attributes'])) $csrf += $this->limits['attributes'];
            $this->limits['fields'] = [ $fn => new Field($csrf) ];

            if($this->limits['reqs']) {
                $this->limits['level'] = $this->limits['reqs'] / $this->limits['requests'];
                if($this->limits['level'] > 1.0) {
                    // add error
                    $this->limits['error'] = S::t('You\'ve sent more requests than allowed in a short period of time. Please wait some time before retrying to send this request.', 'exception');
                } else if($this->limits['level'] >= $this->limits['warn-level']) {
                    // add warning
                    $this->limits['warn'] = S::t('You\'re sending a lot of requests to this page and risks reaching the rate limit for it.', 'exception');
                }
    ...
    public function render($arg=array())
    {
        ...
        // this feature is no longer available in S.js
        /*
        if(App::request('headers', 's-action')=='refreshFields' && ($zp=App::request('headers', 's-params')) && ($fs = json_decode($zp, true))) {
            // check if it's this form
            $r=false;
            if(App::request('headers', 's-target')==$this->id) $r=true;
            else {
                $r=true;
                foreach($fs['f'] as $k) {
        ...
        */
        ...
        if (isset($arg['template'])) {
            if(file_exists($arg['template'])) {
                ...
                $tpl = S::templateFile(preg_replace('/\.php$/', '', $arg['template']), 'form');
            }
            unset($arg['template']);
        }
        ...
        return S::exec([ 'variables' => $vars, 'script' => $tpl ]);
    }
    ...
    private function _value($fn, $d)
    {
        ...
        else if(($slug=S::slug($fn)) && isset($d[$slug])) {
            return $d[$slug];
