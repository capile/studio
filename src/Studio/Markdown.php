...
class Markdown extends Parsedown
{
    ...
    public function __construct()
    {
        if (version_compare(parent::version, '1.7.1') < 0) {
            throw new AppException('ParsedownExtra requires a later version of Parsedown');
        }
    ...
    protected function blockFencedCodeComplete($Block)
    {
        ...
        if(static::$cacheSyntaxHighlight && ($k='hlite'.md5($Block['element']['element']['text'])) && ($text=Cache::get($k, static::$cacheSyntaxHighlight))) {
        ...
        if(static::$syntaxHighlight=='geshi' && class_exists('GeSHi')) {
            if($lang) {
                static $G;
                if(is_null($G)) {
        ...
        if(!isset($text) || $text===false) {
            ...
            if(static::$cacheSyntaxHighlight) {
                Cache::set($k, $text, static::$cacheSyntaxHighlight);
            }
            $Block['element']['element']['rawHtml'] = $text;
            unset($Block['element']['element']['text']);
            if(is_string($Block['highlight'])) {
    ...
    protected function blockAppComplete($Block)
    {
        ...
        if(isset($r['class']) && isset($r['method']) && isset($r['static']) && $r['static']) {
            ...
            if($s = S::getApp()->runRoute($r)) {
                if($s===true) $s = App::response('template');
        ...
        foreach($D->documentElement?->childNodes as $i=>$E) {
            if($i>=1) {
                ...
                if($E instanceof DOMElement) {
                    ...
                    $markup = $this->innerText($E->wholeText);
                }
                $r .= $markup;  
            }
            unset($i, $E);
