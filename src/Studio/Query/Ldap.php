        ...
        if(!isset(static::$conn[$n]) || !static::$conn[$n]) {
            try {
                ...
                if(isset($db['options']['certificate'])) {
                    ...
                    if(substr($crt, 0, 1)!='/') $crt = S_APP_ROOT.'/'.$crt;
                ...
                if(!static::$conn[$n]) {
                    throw new AppException('ldap_connect failed!');
                }
                if($db['options']) {
                    $level = 'configure';
                ...
                if(isset($db['username'])) {
                    ...
                    if(!@ldap_bind(static::$conn[$n], $db['username'], $db['password'])) {
                        throw new AppException(ldap_error(static::$conn[$n]));
                    }
                }
            } catch(Exception $e) {
                S::log('[INFO] Could not '.$level.' to '.$n.": {$e->getMessage()}");
                if($tries) {
                    $tries--;
                    if(isset(static::$conn[$n])) static::$conn[$n]=null;
                    return static::connect($n, $exception, $tries);
                } else {
                    S::log("[WARNING] Giving up on {$level} to {$n}: {$e->getMessage()}");
                }
                if($exception) {
    ...
    public function schema($prop=null, $object=true)
    {
        ...
        if($object) {
            if(!isset(static::$schemas[$cn])) {
                if(!is_object($cn::$schema)) static::$schemas[$cn] = new Schema($cn::$schema);
    ...
    */
            ...
            if(is_int($k)) {
                ...
                if($fn) {
                    ...
                    if (is_array($v) && ($cop=='=' || $cop=='!=' || $cop=='!' || $cop=='<>')) {
                        ...
                        $r .= (($cnot)?('(!('):('('))."{$fn}=*".str_replace('-', '*', S::slug($v, $cn::$queryAllowedChars, true))."*".(($cnot)?('))'):(')'));
                    } else {
    ...
    public function insert($M, $conn=null)
    {
        if($data=$this->valuesToSave($M, true)) {
            ...
            if($r===false) {
                throw new AppException(sprintf(S::t('Could not save %s', 'exception'), $this->getDn($M)).': '.ldap_error($conn));
            }
            return $r;
        }
    }
    ...
    protected function valuesToSave($M, $new=null)
    {
        ...
        foreach($fs as $fn=>$fv) {
            ...
            if($new && (!isset($odata[$fn]) || S::isempty($odata[$fn])) && isset($fv->default)) {
