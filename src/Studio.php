...
class Studio
{
     ...
     * @return PDO
     */
    public static function connect(mixed $db=false, ?string $app=null, bool $throw=false): object|false
    {
     ...
     */
    public static function minify(array|string $s, ?string $root=null, bool $compress=true, bool $before=true, bool $raw=false, bool|string $output=false): string|bool
    {
    ...
    public static function browser(?string $s=null): ?string
    {
    ...
    public static function download(string $file, string|false|null $format=null, ?string $fname=null, int $speed=0, bool $attachment=false, bool $nocache=false, bool $exit=true): void
    {
    ...
    public static function unserialize(string $a, string $hint='', ?int $flags=null): mixed
    {
        if(is_string($a)) {
     ...
     */
    public static function httpAuth(string $pass_file='.htpasswd', string $crypt_type='DES', ?string $user=null, ?string $pass=null): bool
    {
    ...
    public static function encrypt(string $s, ?string $salt=null, ?string $alg=null, bool $encode=true): string
    {
        return Crypto::encrypt($s, $salt, $alg, $encode);
    }

    public static function decrypt(string $r, ?string $salt=null, ?string $alg=null): string
    {
    ...
    public static function hash(string $str, ?string $salt=null, bool|int|string|null $type=40): string
    {
    ...
    public static function dateDiff(string|int|float $start, string|int|float $end='', bool $showtime=false, string $ttable='date'): string
    {
        ...
        if ($start == $end){// same time
            ...
            if($showtime) {
                $str .= ' '.self::t('from', $ttable).' '.date(self::$timeFormat, $tstart)
                . ' '.self::t('up to', $ttable).' '.date(self::$timeFormat, $tend);
            }
        } else if(substr($start, 0, 7) == substr($end, 0, 7) && preg_match('/[dj]/', self::$dateFormat)){// same month
            $b = preg_replace('/([dj]+).*/', '$1', self::$dateFormat);
            $a = preg_replace('/.*([dj]+)/', '$1', self::$dateFormat);
            $str = date($b, $tstart)
                . ' '.self::t('to', $ttable)
                . ' '.date($a, $tend)
                . (($showtime) ?' '.date(self::$timeFormat, $tend) :'');
        } else if (substr($start, 0, 5) == substr($end, 0, 5) && preg_match('@([\s\.\-\/]*[yY]+)$@', self::$dateFormat, $m)) { // same year
            $b = substr(self::$dateFormat, 0, strlen(self::$dateFormat) - strlen($m[0]));
            $str = date($b, $tstart)
                . ' '.self::t('to', $ttable)
    ...
    public static function tune(?string $s='', int|null $m=20, int|null $t=20, bool $allowLessMemory=false): void
    {
        if($m) {
